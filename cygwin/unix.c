...
int microsec;
{
#if defined(USG) && !defined(M_XENIX) && !defined(__linux__) && !defined(__CYGWIN__)
  int arg, result;
#else
  struct timeval tbuf;
  int ch;
#if defined(BSD4_3) || defined(M_XENIX) || defined(__linux__) || defined(__CYGWIN__)
  fd_set smask;
#else
  int smask;
#endif
...
#if !defined(DEBIAN_LINUX) && !defined(__CYGWIN__)
#ifdef USG
